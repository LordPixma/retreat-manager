...
// Enhanced announcement management frontend component
// frontend/public/js/components/enhanced-announcement-management.js

// Extend the existing AnnouncementManagement with email functionality
if (window.AnnouncementManagement) {
  // Store the original methods
  const originalSubmitForm = window.AnnouncementManagement.submitForm;
  const originalRenderModal = window.AnnouncementManagement.renderModal;

  // Enhance the form submission to include email options
  window.AnnouncementManagement.submitForm = async function() {
    const sendEmail = document.getElementById('announcement-send-email')?.checked;
    
    // Call original submit
    await originalSubmitForm.call(this);
    
    // If email is enabled and the form was successful, send emails
    if (sendEmail && !this.hasError) {
      try {
        const announcementId = this.lastCreatedId; // Assume we store this
        if (announcementId) {
          const result = await API.post(`/admin/announcements/${announcementId}/email`);
          Utils.showAlert(`Announcement created and emailed to ${result.results.successful} attendees!`, 'success');
        }
      } catch (error) {
        console.error('Failed to send announcement emails:', error);
        Utils.showAlert('Announcement created but email sending failed: ' + error.message, 'warning');
      }
    }
  };

  // Enhance the modal to include email options
  window.AnnouncementManagement.renderModal = function() {
    // Call original render
    originalRenderModal.call(this);
    
    // Add email options to the form
    const formContent = document.querySelector('#announcement-modal .modal-body form');
    if (formContent) {
      const emailSection = `
        <div class="form-group" style="border-top: 1px solid var(--border); padding-top: 1.5rem; margin-top: 1.5rem;">
          <div class="form-checkbox-group">
            <label class="checkbox-item">
              <input type="checkbox" id="announcement-send-email" name="send_email">
              <span class="checkbox-label">
                <strong>📧 Send as Email</strong>
                <small style="display: block; color: var(--text-secondary); margin-top: 0.25rem;">
                  Also send this announcement to attendees via email (based on target audience)
                </small>
              </span>
            </label>
          </div>
      ...
      if (formActions) {
        formActions.insertAdjacentHTML('beforebegin', emailSection);
      }
    }
  };

  // Add email sending functionality to existing announcements
  window.AnnouncementManagement.sendAnnouncementEmail = async function(announcementId) {
    try {
      const result = await API.post(`/admin/announcements/${announcementId}/email`);
      Utils.showAlert(`Email sent to ${result.results.successful} attendees!`, 'success');
      return result;
    } catch (error) {
      console.error('Failed to send announcement email:', error);
      Utils.showAlert('Failed to send announcement email: ' + error.message, 'error');
      throw error;
    }
  };
}