...
// .wrangler/tmp/bundle-3iYsHW/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-B79zuF/functionsWorker-0.4514949215307493.mjs
...
var init_auth = __esm({
  "_shared/auth.ts"() {
    ...
    __name2(legacyHashPassword, "legacyHashPassword");
    ...
    __name2(generateAdminToken, "generateAdminToken");
    __name2(generateAttendeeToken, "generateAttendeeToken");
    __name2(checkAdminAuth, "checkAdminAuth");
    __name2(checkAttendeeAuth, "checkAttendeeAuth");
    __name2(checkRateLimit, "checkRateLimit");
    __name2(recordLoginAttempt, "recordLoginAttempt");
    __name2(clearRateLimit, "clearRateLimit");
    __name2(createResponse, "createResponse");
    __name2(handleCORS, "handleCORS");
    __name2(needsPasswordUpgrade, "needsPasswordUpgrade");
  }
});
...
function handleError(error32, requestId) {
  if (error32 instanceof AppError) {
    if (requestId) {
      error32.requestId = requestId;
    }
    return error32;
  }
  if (error32 instanceof Error) {
    if (error32.message.includes("UNIQUE constraint failed")) {
      return errors.conflict("Resource already exists", requestId);
    }
    if (error32.message.includes("FOREIGN KEY constraint failed")) {
      return errors.badRequest("Referenced resource does not exist", requestId);
    }
    if (error32.message.includes("NOT NULL constraint failed")) {
      return errors.badRequest("Required field is missing", requestId);
    }
    return errors.internal(error32.message, requestId);
  }
...
var init_errors = __esm({
  "_shared/errors.ts"() {
    ...
    errors = {
      validation: (fields, requestId) => new AppError("VALIDATION_ERROR", "Validation failed", { fields }, requestId),
      unauthorized: (message = "Unauthorized", requestId) => new AppError("UNAUTHORIZED", message, void 0, requestId),
      forbidden: (message = "Forbidden", requestId) => new AppError("FORBIDDEN", message, void 0, requestId),
      notFound: (resource, requestId) => new AppError("NOT_FOUND", `${resource} not found`, void 0, requestId),
      conflict: (message, requestId) => new AppError("CONFLICT", message, void 0, requestId),
      badRequest: (message, requestId) => new AppError("BAD_REQUEST", message, void 0, requestId),
      internal: (message = "Internal server error", requestId) => new AppError("INTERNAL_ERROR", message, void 0, requestId),
      database: (message = "Database error", requestId) => new AppError("DATABASE_ERROR", message, void 0, requestId),
      rateLimited: (retryAfter, requestId) => new AppError("RATE_LIMITED", "Rate limit exceeded", { retryAfter }, requestId),
      externalService: (service, requestId, details) => new AppError("EXTERNAL_SERVICE_ERROR", details ? `${service} error: ${details}` : `${service} service error`, void 0, requestId)
    };
...
async function onRequestPost(context22) {
  ...
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
      return createErrorResponse(errors.unauthorized("Invalid or expired token", requestId));
    }
    const announcementId = context22.params.id;
    if (!announcementId) {
      return createErrorResponse(errors.badRequest("Announcement ID is required", requestId));
    }
    const { results: announcements } = await context22.env.DB.prepare(`
    ...
    const { results: attendees } = await context22.env.DB.prepare(query).bind(...bindings).all();
    if (attendees.length === 0) {
      return createErrorResponse(errors.badRequest("No attendees found matching announcement criteria", requestId));
    }
    if (!context22.env.RESEND_API_KEY || !context22.env.FROM_EMAIL) {
    ...
    const emailResults = await sendAnnouncementEmails(context22.env, {
      announcement,
      attendees,
      adminUser: admin.user || "Admin"
    });
    await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Error sending announcement email:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPost, "onRequestPost");
async function sendAnnouncementEmails(env22, options) {
  const { announcement, attendees, adminUser } = options;
  if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL) {
  ...
  for (let i = 0; i < attendees.length; i += batchSize) {
    ...
    await Promise.all(batch.map(async (attendee) => {
      try {
        ...
        results.failed++;
        results.errors.push(`${attendee.name}: ${error32 instanceof Error ? error32.message : "Unknown error"}`);
      }
    }));
...
function generateAnnouncementEmailTemplate(options) {
  ...
  const getHeaderStyle = /* @__PURE__ */ __name2((type) => {
    switch (type) {
      case "urgent":
  ...
  const getIcon = /* @__PURE__ */ __name2((type) => {
    switch (type) {
      case "urgent":
        return "URGENT";
      case "event":
  ...
  const getPriorityBadge2 = /* @__PURE__ */ __name2((priority) => {
    if (priority >= 4) {
...
var init_email = __esm({
  "api/admin/announcements/[id]/email.ts"() {
    ...
    __name2(sendAnnouncementEmails, "sendAnnouncementEmails");
    __name2(generateAnnouncementEmailTemplate, "generateAnnouncementEmailTemplate");
  }
});
function validate(data, schema) {
  ...
  for (const [fieldName, fieldSchema] of Object.entries(schema)) {
    ...
    for (const validatorFn of fieldSchema.validators) {
      const error32 = validatorFn(value, fieldName);
      if (error32) {
        errors2[fieldName] = error32;
        break;
      }
    }
  }
...
var validators;
var attendeeCreateSchema;
var attendeeUpdateSchema;
var roomCreateSchema;
var roomUpdateSchema;
var groupCreateSchema;
var groupUpdateSchema;
var announcementCreateSchema;
var announcementUpdateSchema;
var emailSendSchema;
var individualEmailSchema;
var loginSchema;
var adminLoginSchema;
...
async function onRequestPost2(context22) {
  ...
  try {
    ...
    const { results: attendees } = await context22.env.DB.prepare(`
    ...
    const response = await fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${context22.env.RESEND_API_KEY}`,
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
        from: context22.env.FROM_EMAIL,
        to: [attendee.email],
        subject,
        html: emailContent
      })
    ...
    console.error(`[${requestId}] Email sending error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
var init_individual = __esm({
  "api/admin/email/individual.ts"() {
    ...
    __name2(generateEmailContent, "generateEmailContent");
  }
});
var headerStyles;
var typeIcons;
...
async function onRequestPost3(context22) {
  ...
  try {
    ...
    const { notification_type, attendee_id, custom_data } = await context22.request.json();
    if (!notification_type) {
      return createErrorResponse(errors.badRequest("Notification type is required", requestId));
    }
    const result = await sendNotification(context22.env, {
      notification_type,
      attendee_id,
      custom_data,
    ...
    console.error(`[${requestId}] Error sending notification:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPost3, "onRequestPost3");
async function sendNotification(env22, options) {
  ...
  switch (notification_type) {
    case "welcome":
      attendees = await getAttendeeById(env22.DB, attendee_id);
    ...
    case "payment_reminder":
      if (attendee_id) {
        ...
        attendees = await getAttendeesWithOutstandingPayments(env22.DB);
      }
      emailTemplate = generatePaymentReminderEmail(attendees[0] || null, custom_data);
      break;
    case "announcement_urgent":
      attendees = await getAttendeesByAnnouncement(env22.DB, custom_data || {});
  ...
  for (const attendee of attendees) {
    ...
    try {
      ...
      results.failed++;
      results.errors.push(`${attendee.name}: ${error32.message}`);
    }
  }
  return {
    success: true,
...
var init_notifications = __esm({
  "api/admin/email/notifications.ts"() {
    ...
    __name2(getAttendeeById, "getAttendeeById");
    __name2(getAttendeesWithOutstandingPayments, "getAttendeesWithOutstandingPayments");
    __name2(getAttendeesByAnnouncement, "getAttendeesByAnnouncement");
    __name2(generateWelcomeEmail, "generateWelcomeEmail");
    __name2(generatePaymentReminderEmail, "generatePaymentReminderEmail");
    __name2(generateAnnouncementEmail, "generateAnnouncementEmail");
    __name2(generateRoomAssignmentEmail, "generateRoomAssignmentEmail");
    __name2(generateGroupAssignmentEmail, "generateGroupAssignmentEmail");
    __name2(generateGenericTemplate, "generateGenericTemplate");
  }
});
...
async function onRequestPost4(context22) {
  ...
  try {
    ...
    const attendees = await getTargetAttendees(context22.env.DB, {
      target_audience,
      target_groups,
      attendee_ids
    });
    ...
    const emailResults = await sendBulkEmails(context22.env, {
      attendees,
      subject,
      message,
      email_type,
    ...
    console.error(`[${requestId}] Error sending bulk emails:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function sendBulkEmails(env22, emailData) {
  if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL) {
    throw new Error("Email service not configured");
  }
  ...
  for (let i = 0; i < attendees.length; i += batchSize) {
    ...
    await Promise.all(batch.map(async (attendee) => {
      try {
        ...
        results.failed++;
        results.errors.push(`${attendee.name} (${attendee.email}): ${error32.message}`);
      }
    }));
    if (i + batchSize < attendees.length) {
      await new Promise((resolve) => setTimeout(resolve, 1e3));
...
var init_send = __esm({
  "api/admin/email/send.ts"() {
    ...
    __name2(getTargetAttendees, "getTargetAttendees");
    __name2(sendBulkEmails, "sendBulkEmails");
    __name2(generateEmailTemplate, "generateEmailTemplate");
  }
});
...
async function onRequestPost5(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Auto-create groups error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
var init_HttpClient = __esm({
  "../node_modules/stripe/esm/net/HttpClient.js"() {
    ...
    HttpClientResponse = /* @__PURE__ */ __name(class {
...
var init_FetchHttpClient = __esm({
  "../node_modules/stripe/esm/net/FetchHttpClient.js"() {
    ...
    init_utils2();
    init_HttpClient();
    FetchHttpClient = /* @__PURE__ */ __name(class extends HttpClient {
      constructor(fetchFn) {
        super();
        if (!fetchFn) {
          if (!globalThis.fetch) {
    ...
    FetchHttpClientResponse = /* @__PURE__ */ __name(class extends HttpClientResponse {
      constructor(res) {
...
var init_CryptoProvider = __esm({
  "../node_modules/stripe/esm/crypto/CryptoProvider.js"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    CryptoProvider = /* @__PURE__ */ __name(class {
      /**
    ...
    CryptoProviderOnlySupportsAsyncError = /* @__PURE__ */ __name(class extends Error {
...
var init_SubtleCryptoProvider = __esm({
  "../node_modules/stripe/esm/crypto/SubtleCryptoProvider.js"() {
    ...
    init_performance2();
    init_CryptoProvider();
    SubtleCryptoProvider = /* @__PURE__ */ __name(class extends CryptoProvider {
...
var init_PlatformFunctions = __esm({
  "../node_modules/stripe/esm/platform/PlatformFunctions.js"() {
    ...
    PlatformFunctions = /* @__PURE__ */ __name(class {
      constructor() {
        this._fetchFn = null;
        this._agent = null;
      }
...
var init_StripeEmitter = __esm({
  "../node_modules/stripe/esm/StripeEmitter.js"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    _StripeEvent = /* @__PURE__ */ __name(class extends Event {
    ...
    StripeEmitter = /* @__PURE__ */ __name(class {
      constructor() {
...
var init_WebPlatformFunctions = __esm({
  "../node_modules/stripe/esm/platform/WebPlatformFunctions.js"() {
    ...
    init_StripeEmitter();
    WebPlatformFunctions = /* @__PURE__ */ __name(class extends PlatformFunctions {
      /** @override */
      getUname() {
...
var init_Error = __esm({
  "../node_modules/stripe/esm/Error.js"() {
    ...
    StripeError = /* @__PURE__ */ __name(class extends Error {
    ...
    StripeCardError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeCardError");
      }
    }, "StripeCardError");
    __name2(StripeCardError, "StripeCardError");
    StripeInvalidRequestError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeInvalidRequestError");
      }
    }, "StripeInvalidRequestError");
    __name2(StripeInvalidRequestError, "StripeInvalidRequestError");
    StripeAPIError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeAPIError");
      }
    }, "StripeAPIError");
    __name2(StripeAPIError, "StripeAPIError");
    StripeAuthenticationError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeAuthenticationError");
      }
    }, "StripeAuthenticationError");
    __name2(StripeAuthenticationError, "StripeAuthenticationError");
    StripePermissionError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripePermissionError");
      }
    }, "StripePermissionError");
    __name2(StripePermissionError, "StripePermissionError");
    StripeRateLimitError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeRateLimitError");
      }
    }, "StripeRateLimitError");
    __name2(StripeRateLimitError, "StripeRateLimitError");
    StripeConnectionError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeConnectionError");
      }
    }, "StripeConnectionError");
    __name2(StripeConnectionError, "StripeConnectionError");
    StripeSignatureVerificationError = /* @__PURE__ */ __name(class extends StripeError {
    ...
    StripeIdempotencyError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeIdempotencyError");
      }
    }, "StripeIdempotencyError");
    __name2(StripeIdempotencyError, "StripeIdempotencyError");
    StripeInvalidGrantError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeInvalidGrantError");
      }
    }, "StripeInvalidGrantError");
    __name2(StripeInvalidGrantError, "StripeInvalidGrantError");
    StripeUnknownError = /* @__PURE__ */ __name(class extends StripeError {
      constructor(raw = {}) {
        super(raw, "StripeUnknownError");
      }
    }, "StripeUnknownError");
    __name2(StripeUnknownError, "StripeUnknownError");
    TemporarySessionExpiredError = /* @__PURE__ */ __name(class extends StripeError {
...
var init_RequestSender = __esm({
  "../node_modules/stripe/esm/RequestSender.js"() {
    ...
    RequestSender = /* @__PURE__ */ __name(class {
        ...
        const makeRequest = /* @__PURE__ */ __name2((apiVersion, headers, numRetries) => {
          ...
          authenticator(request).then(() => {
            ...
            req.then((res) => {
              ...
              if (RequestSender._shouldRetry(null, requestRetries, maxRetries, error32)) {
...
var init_autoPagination = __esm({
  "../node_modules/stripe/esm/autoPagination.js"() {
    ...
    init_performance2();
    init_utils2();
    V1Iterator = /* @__PURE__ */ __name(class {
    ...
    V1ListIterator = /* @__PURE__ */ __name(class extends V1Iterator {
    ...
    V1SearchIterator = /* @__PURE__ */ __name(class extends V1Iterator {
      getNextPage(pageResult) {
    ...
    V2ListIterator = /* @__PURE__ */ __name(class {
      constructor(firstPagePromise, requestArgs, spec, stripeResource) {
...
var init_StripeContext = __esm({
  "../node_modules/stripe/esm/StripeContext.js"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    StripeContext = /* @__PURE__ */ __name(class {
      /**
...
function getStripe(env22) {
  if (!env22.STRIPE_SECRET_KEY) {
    throw new Error("STRIPE_SECRET_KEY is not configured");
  }
  if (!stripeInstance) {
    stripeInstance = new stripe_esm_worker_default(env22.STRIPE_SECRET_KEY, {
...
var init_stripe = __esm({
  "_shared/stripe.ts"() {
    ...
    stripeInstance = null;
    __name2(getStripe, "getStripe");
    __name2(poundsToPence, "poundsToPence");
    __name2(penceToPounds, "penceToPounds");
    __name2(createStripeCustomer, "createStripeCustomer");
    __name2(createCheckoutSession, "createCheckoutSession");
    __name2(verifyWebhookEvent, "verifyWebhookEvent");
  }
});
...
async function onRequestPost6(context22) {
  ...
  try {
    ...
    const portalUrl = context22.env.PORTAL_URL || "https://retreat.cloverleafchristiancentre.org";
    const retreatName = context22.env.RETREAT_NAME || "Growth and Wisdom Retreat";
    const { results: schedules } = await context22.env.DB.prepare(`
    ...
    const stripe = getStripe(context22.env);
    let sent = 0;
    const results = [];
    for (const row of schedules) {
      ...
      try {
        ...
        console.error(`[${requestId}] Failed to process installment for ${schedule.name}:`, error32);
        results.push({ attendee: schedule.name, status: "error", error: String(error32) });
      }
    }
    return createResponse({
    ...
    console.error(`[${requestId}] Installment reminder error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost7(context22) {
  ...
  try {
    ...
    const retreatName = context22.env.RETREAT_NAME || "Growth and Wisdom Retreat";
    const portalUrl = context22.env.PORTAL_URL || "https://retreat.cloverleafchristiancentre.org";
    const { results } = await context22.env.DB.prepare(`
    ...
    for (const row of results) {
      try {
        await fetch("https://api.resend.com/emails", {
          method: "POST",
          headers: { "Authorization": `Bearer ${context22.env.RESEND_API_KEY}`, "Content-Type": "application/json" },
          body: JSON.stringify({
            from: context22.env.FROM_EMAIL,
            to: [row.email],
    ...
    console.error(`[${requestId}] Payment reminder error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet(context22) {
  ...
  try {
    ...
    const { results: collectedRows } = await context22.env.DB.prepare(`
      SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
      FROM payments WHERE status = 'succeeded'
    `).all();
    const { results: outstandingRows } = await context22.env.DB.prepare(`
      SELECT COALESCE(SUM(payment_due), 0) as total, COUNT(*) as count
      FROM attendees WHERE payment_due > 0
    `).all();
    const { results: byStatusRows } = await context22.env.DB.prepare(`
      SELECT status, COUNT(*) as count, COALESCE(SUM(amount), 0) as total
      FROM payments GROUP BY status
    `).all();
    const { results: byTypeRows } = await context22.env.DB.prepare(`
    ...
    `).all();
    const { results: installmentRows } = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Payment summary error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet2(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Analytics error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
var init_pagination = __esm({
  "_shared/pagination.ts"() {
    ...
    MAX_LIMIT = 500;
    MIN_LIMIT = 1;
    __name2(parsePaginationParams, "parsePaginationParams");
    __name2(createPaginationMeta, "createPaginationMeta");
    __name2(createPaginatedResponse, "createPaginatedResponse");
  }
});
...
async function onRequestGet3(context22) {
  ...
  try {
    ...
    const { results: countResult } = await context22.env.DB.prepare(
    ...
    console.error(`[${requestId}] Error fetching login history:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet4(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching activity teams:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost8(context22) {
  ...
  try {
    ...
    const { results: existing } = await context22.env.DB.prepare(
    ...
    const result = await context22.env.DB.prepare(`
    ...
    context22.waitUntil(emailPromise);
    return createResponse({ id: teamId, message: "Activity team created successfully" }, 201);
  } catch (error32) {
    console.error(`[${requestId}] Error creating activity team:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPost8, "onRequestPost8");
async function sendTeamNotificationEmails(env22, teamId, teamName, description, memberIds, leaderId, requestId) {
  if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL || memberIds.length === 0)
    return;
  const retreatName = env22.RETREAT_NAME || "Growth and Wisdom Retreat";
  ...
  if (leaderId) {
    const { results: leaderRows } = await env22.DB.prepare(
...
var init_activity_teams = __esm({
  "api/admin/activity-teams/index.ts"() {
    ...
    __name2(sendTeamNotificationEmails, "sendTeamNotificationEmails");
  }
});
...
async function onRequestGet5(context22) {
  ...
  try {
    ...
    const id = context22.params.id;
    const { results: teamRows } = await context22.env.DB.prepare(`
    ...
    const { results: members } = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Error fetching activity team:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPut(context22) {
  ...
  try {
    ...
    const body = await context22.request.json();
    if (body.name) {
      const { results: nameCheck } = await context22.env.DB.prepare(
    ...
    if (body.member_ids !== void 0) {
      const { results: currentMembers } = await context22.env.DB.prepare(
      ...
      if (addedIds.length > 0) {
        const teamName = body.name?.trim() || (await context22.env.DB.prepare("SELECT name FROM activity_teams WHERE id = ?").bind(id).all()).results[0];
    ...
    console.error(`[${requestId}] Error updating activity team:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPut, "onRequestPut");
async function onRequestDelete(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
    ...
    await context22.env.DB.prepare("DELETE FROM activity_teams WHERE id = ?").bind(id).run();
    return createResponse({ message: `Team "${results[0].name}" deleted successfully` });
  } catch (error32) {
    console.error(`[${requestId}] Error deleting activity team:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet6(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching announcement:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPut2(context22) {
  ...
  try {
    ...
    const id = context22.params.id;
    const updateData = await context22.request.json();
    ...
    const result = await context22.env.DB.prepare(updateQuery).bind(...updateValues).run();
    if (!result.success) {
    ...
    console.error(`[${requestId}] Error updating announcement:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestDelete2(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error deleting announcement:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet7(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching attendee:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPut3(context22) {
  ...
  try {
    ...
    const { results: existingResults } = await context22.env.DB.prepare(
    ...
    console.error(`[${requestId}] Error updating attendee:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPut3, "onRequestPut3");
async function onRequestDelete3(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
    ...
    console.error(`[${requestId}] Error deleting attendee:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet8(context22) {
  ...
  try {
    ...
    const group32 = results[0];
    const memberNames = group32.members ? group32.members.split(", ").filter(Boolean) : [];
    const memberRefs = group32.member_refs ? group32.member_refs.split(", ").filter(Boolean) : [];
    ...
    const formattedGroup = {
      id: group32.id,
      name: group32.name,
      member_count: group32.member_count || 0,
      members
    };
    return createResponse(formattedGroup);
  } catch (error32) {
    console.error(`[${requestId}] Error fetching group:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPut4(context22) {
  ...
  try {
    ...
    const { results: conflict } = await context22.env.DB.prepare(
    ...
    console.error(`[${requestId}] Error updating group:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestDelete4(context22) {
  ...
  try {
    ...
    return createResponse({
      success: true,
      message: `Group ${group32.name} deleted successfully`
    });
  } catch (error32) {
    console.error(`[${requestId}] Error deleting group:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPut5(context22) {
  ...
  try {
    ...
    if (body.action === "confirm") {
      ...
      const { results: attendeeRows } = await context22.env.DB.prepare(
      ...
      context22.waitUntil(sendConfirmation(context22.env, payment.attendee_id, payment.amount, requestId));
    ...
    console.error(`[${requestId}] Payment action error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPut5, "onRequestPut5");
async function sendConfirmation(env22, attendeeId, amountPence, requestId) {
  try {
    if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL)
      return;
    const { results } = await env22.DB.prepare("SELECT name, email FROM attendees WHERE id = ?").bind(attendeeId).all();
    if (results.length === 0)
      return;
    const attendee = results[0];
    if (!attendee.email)
...
var init_id5 = __esm({
  "api/admin/payments/[id].ts"() {
    ...
    __name2(sendConfirmation, "sendConfirmation");
  }
});
...
var init_names = __esm({
  "_shared/names.ts"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    __name2(splitFullName, "splitFullName");
    __name2(ageFromDateOfBirth, "ageFromDateOfBirth");
  }
});
...
async function onRequestGet10(context22) {
  ...
  try {
    ...
    const registrationId = context22.params.id;
    if (!registrationId) {
    ...
    console.error(`[${requestId}] Error fetching registration:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestGet10, "onRequestGet10");
async function onRequestPut6(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
    ...
    if (body.action === "approve") {
      ...
      for (let i = 0; i < familyMembers.length; i++) {
        ...
        const refNumber = await generateRefNumber(context22.env.DB);
        const tempPassword = generateTempPassword();
        const passwordHash = await hashPassword(tempPassword);
        const isPrimary = i === 0;
        const memberName = member.name.trim();
        const { first: firstName, last: lastName } = splitFullName(memberName);
        const attendeeResult = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Error updating registration:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPut6, "onRequestPut6");
async function onRequestDelete5(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
    ...
    console.error(`[${requestId}] Error deleting registration:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function sendApprovalEmail(env22, email, primaryName, attendees, totalAmount, paymentOption) {
  if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL) {
    console.warn("Email service not configured - skipping approval email");
    return;
  }
  const portalUrl = env22.PORTAL_URL || "https://retreat-manager.pages.dev";
  const retreatName = env22.RETREAT_NAME || "Growth and Wisdom Retreat";
  const paymentLabels = {
    full: "Pay in Full",
...
var init_id6 = __esm({
  "api/admin/registrations/[id].ts"() {
    ...
    __name2(generateRefNumber, "generateRefNumber");
    __name2(generateTempPassword, "generateTempPassword");
    __name2(sendApprovalEmail, "sendApprovalEmail");
  }
});
...
async function onRequestGet11(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching room:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestGet11, "onRequestGet11");
async function onRequestPut7(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
    ...
    console.error(`[${requestId}] Error updating room:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestDelete6(context22) {
  ...
  try {
    ...
    const { results: occupants } = await context22.env.DB.prepare(
    ...
    console.error(`[${requestId}] Error deleting room:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet12(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching announcements:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost9(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error creating announcement:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet13(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching attendees:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost10(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error creating attendee:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet14(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Audit log error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost11(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Bulk action error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost12(context22) {
  ...
  try {
    ...
    const bind = ref || id;
    const { results } = await context22.env.DB.prepare(query).bind(bind).all();
    if (results.length === 0) {
      return createErrorResponse(errors.notFound("Attendee", requestId));
    }
    const attendee = results[0];
    ...
    console.error(`[${requestId}] Check-in error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet15(context22) {
  ...
  try {
    ...
    if (!admin)
      return createErrorResponse(errors.unauthorized("Invalid or expired token", requestId));
    const { results: stats } = await context22.env.DB.prepare(`
      SELECT
        COUNT(*) as total,
    ...
    `).all();
    const { results: recent } = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Check-in stats error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet16(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin)
    ...
    console.error(`[${requestId}] Export error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet17(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
    ...
    const { results } = await context22.env.DB.prepare(`
      ...
      LIMIT ? OFFSET ?
    `).bind(limit, offset).all();
    const formattedGroups = results.map((group32) => {
      const memberNames = group32.members ? group32.members.split(", ").filter(Boolean) : [];
      const memberRefs = group32.member_refs ? group32.member_refs.split(", ").filter(Boolean) : [];
      const members = memberNames.map((name, index) => ({
        name,
      ...
      return {
        ...
        members,
        financial: {
          totalOutstanding: group32.total_outstanding || 0,
          membersWithPayments: group32.members_with_payments || 0
        }
      };
    });
    return createResponse(createPaginatedResponse(formattedGroups, total, limit, offset));
  } catch (error32) {
    console.error(`[${requestId}] Error fetching groups:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost13(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error creating group:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost14(context22) {
  ...
  try {
    ...
    const clientIP = context22.request.headers.get("CF-Connecting-IP") || context22.request.headers.get("X-Forwarded-For") || "unknown";
    const rateLimit = await checkRateLimit(context22.env.DB, trimmedUser, "admin");
    if (!rateLimit.allowed) {
      return createErrorResponse(errors.rateLimited(Math.ceil((rateLimit.resetTime - Date.now()) / 1e3), requestId));
    }
    const adminUser = context22.env.ADMIN_USER || "admin";
    const adminPass = context22.env.ADMIN_PASS || "admin123";
    const userMatch = timingSafeEqual2(trimmedUser, adminUser);
    const passMatch = timingSafeEqual2(pass, adminPass);
    if (!userMatch || !passMatch) {
      await recordLoginAttempt(context22.env.DB, trimmedUser, "admin", false, clientIP);
      return createErrorResponse(errors.unauthorized("Invalid credentials", requestId));
    }
    await recordLoginAttempt(context22.env.DB, trimmedUser, "admin", true, clientIP);
    await clearRateLimit(context22.env.DB, trimmedUser, "admin");
    await context22.env.DB.prepare(`
      INSERT INTO login_history (user_type, user_id, login_time)
      VALUES ('admin', ?, CURRENT_TIMESTAMP)
    `).bind(trimmedUser).run();
    const token = await generateAdminToken(trimmedUser, "admin", context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    return createResponse({ token });
  } catch (error32) {
    console.error(`[${requestId}] Error in admin login:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
__name(timingSafeEqual2, "timingSafeEqual2");
var init_login = __esm({
  "api/admin/login.ts"() {
    ...
    __name2(timingSafeEqual2, "timingSafeEqual");
  }
});
...
async function onRequestGet18(context22) {
  const requestId = generateRequestId();
  try {
    const admin = await checkAdminAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!admin) {
...
async function onRequestGet19(context22) {
  ...
  try {
    ...
    const countStmt = statusFilter ? context22.env.DB.prepare(countQuery).bind(statusFilter) : context22.env.DB.prepare(countQuery);
    const { results: countResult } = await countStmt.all();
    const total = countResult[0].total;
    const stmt = statusFilter ? context22.env.DB.prepare(dataQuery).bind(statusFilter, limit, offset) : context22.env.DB.prepare(dataQuery).bind(limit, offset);
    const { results } = await stmt.all();
    return createResponse(createPaginatedResponse(results, total, limit, offset));
  } catch (error32) {
    console.error(`[${requestId}] Error fetching registrations:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet20(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching rooms:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost15(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error creating room:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost16(context22) {
  ...
  try {
    ...
    console.log(`[${requestId}] API key available:`, !!context22.env.RESEND_API_KEY);
    console.log(`[${requestId}] From email:`, context22.env.FROM_EMAIL || "Not set");
    if (!context22.env.RESEND_API_KEY || !context22.env.FROM_EMAIL) {
      console.error(`[${requestId}] Email configuration missing`);
      return createErrorResponse(errors.internal("Email system not configured", requestId));
    }
    const emailResult = await sendTestEmail(context22.env, testEmail);
    if (emailResult.success) {
      return createResponse({
        success: true,
        message: `Test email sent successfully to ${testEmail}`,
        service: "Resend API",
        fromEmail: context22.env.FROM_EMAIL
      });
    } else {
      return createErrorResponse(errors.externalService("Email service", requestId, emailResult.error));
    }
  } catch (error32) {
    console.error(`[${requestId}] Error sending test email:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
__name(onRequestPost16, "onRequestPost16");
async function sendTestEmail(env22, toEmail) {
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: generateTestEmailTemplate(env22.FROM_EMAIL)
      })
    });
    if (response.ok) {
      const result = await response.json();
    ...
    console.error("Email sending error:", error32);
    return { success: false, error: `Connection error: ${error32 instanceof Error ? error32.message : "Unknown error"}` };
  }
}
...
var init_test = __esm({
  "api/email/test.ts"() {
    ...
    __name2(sendTestEmail, "sendTestEmail");
    __name2(generateTestEmailTemplate, "generateTestEmailTemplate");
  }
});
...
async function onRequestPost17(context22) {
  const requestId = generateRequestId();
  try {
    const auth = await checkAttendeeAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!auth) {
      return createErrorResponse(errors.unauthorized("Invalid or expired token", requestId));
    }
    const body = await context22.request.json();
    ...
    if (body.payment_type === "full") {
      ...
      const count32 = body.installment_count || 3;
      installmentTotal = count32;
      const { results: scheduleRows } = await context22.env.DB.prepare(
      ...
      if (scheduleRows.length > 0) {
        ...
        const perInstallment = Math.ceil(totalPence / count32);
        installmentNumber = 1;
        ...
        await context22.env.DB.prepare(`
          ...
          VALUES (?, ?, ?, ?, 0, 'active', ?)
        `).bind(attendee.id, totalPence, count32, perInstallment, nextDue.toISOString().split("T")[0]).run();
    ...
    console.error(`[${requestId}] Bank transfer error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost18(context22) {
  ...
  try {
    ...
    if (body.payment_type === "full") {
      ...
      const count32 = body.installment_count;
      installmentTotal = count32;
      const { results: scheduleRows } = await context22.env.DB.prepare(
        "SELECT * FROM installment_schedules WHERE attendee_id = ? AND status = ?"
      ).bind(attendee.id, "active").all();
      if (scheduleRows.length > 0) {
        ...
        await context22.env.DB.prepare(`
          ...
          VALUES (?, ?, ?, ?, 0, 'active', ?, ?)
        `).bind(attendee.id, totalPence, count32, perInstallment, nextDueStr, customerId).run();
        amount = perInstallment;
      }
    ...
    console.error(`[${requestId}] Checkout error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet21(context22) {
  ...
  try {
    ...
    const { results: payments } = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Payment history error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestGet22(context22) {
  ...
  try {
    ...
    const attendee = results[0];
    const { results: paidRows } = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Payment status error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost19(context22) {
  ...
  try {
    const stripe = getStripe(context22.env);
    const webhookSecret = context22.env.STRIPE_WEBHOOK_SECRET;
    if (!webhookSecret) {
      console.error(`[${requestId}] STRIPE_WEBHOOK_SECRET not configured`);
      return new Response("Webhook secret not configured", { status: 500 });
    }
    const signature = context22.request.headers.get("stripe-signature");
    if (!signature) {
      return new Response("Missing stripe-signature header", { status: 400 });
    }
    const rawBody = await context22.request.text();
    let event;
    try {
    ...
    switch (event.type) {
      case "checkout.session.completed":
        await handleCheckoutCompleted(context22, event.data.object, requestId);
        break;
      case "checkout.session.expired":
        await handleCheckoutExpired(context22, event.data.object, requestId);
        break;
      default:
    ...
    console.error(`[${requestId}] Webhook error:`, error32);
    return new Response("Webhook handler failed", { status: 500 });
  }
}
__name(onRequestPost19, "onRequestPost19");
async function handleCheckoutCompleted(context22, session, requestId) {
  ...
  context22.waitUntil(sendPaymentConfirmationEmail(context22.env, attendeeId, amountTotal, paymentType, requestId));
}
__name(handleCheckoutCompleted, "handleCheckoutCompleted");
async function handleCheckoutExpired(context22, session, requestId) {
  await context22.env.DB.prepare(`
...
async function sendPaymentConfirmationEmail(env22, attendeeId, amountPence, paymentType, requestId) {
  try {
    if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL)
      return;
    const { results } = await env22.DB.prepare(
    ...
    console.error(`[${requestId}] Failed to send payment confirmation email:`, error32);
  }
}
...
var init_stripe2 = __esm({
  "api/webhooks/stripe.ts"() {
    ...
    __name2(handleCheckoutExpired, "handleCheckoutExpired");
...
async function onRequestGet23(context22) {
  const requestId = generateRequestId();
  try {
    const attendee = await checkAttendeeAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!attendee) {
      return createErrorResponse(errors.unauthorized("Invalid or expired token", requestId));
    }
    const ref = attendee.ref;
    const { results: attendeeInfo } = await context22.env.DB.prepare(`
      SELECT
        a.group_id,
        g.name as group_name
      FROM attendees a
    ...
    console.error(`[${requestId}] Error fetching announcements for attendee:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
var init_announcements2 = __esm({
  "api/announcements.ts"() {
    ...
    __name2(isNewAnnouncement, "isNewAnnouncement");
  }
});
async function onRequestOptions37() {
  return handleCORS();
}
__name(onRequestOptions37, "onRequestOptions37");
async function onRequestPost20(context22) {
  ...
  try {
    ...
    const clientIP = context22.request.headers.get("CF-Connecting-IP") || context22.request.headers.get("X-Forwarded-For") || "unknown";
    const rateLimit = await checkRateLimit(context22.env.DB, trimmedRef, "attendee");
    if (!rateLimit.allowed) {
      return createErrorResponse(errors.rateLimited(Math.ceil((rateLimit.resetTime - Date.now()) / 1e3), requestId));
    }
    const { results } = await context22.env.DB.prepare(`
    ...
    `).bind(trimmedRef).all();
    if (!results.length) {
      await recordLoginAttempt(context22.env.DB, trimmedRef, "attendee", false, clientIP);
      return createErrorResponse(errors.unauthorized("Invalid credentials", requestId));
    }
    const attendee = results[0];
    ...
    await recordLoginAttempt(context22.env.DB, trimmedRef, "attendee", true, clientIP);
    await clearRateLimit(context22.env.DB, trimmedRef, "attendee");
    ...
    await context22.env.DB.prepare(`
      ...
      WHERE ref_number = ?
    `).bind(trimmedRef).run();
    const token = await generateAttendeeToken(trimmedRef, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    return createResponse({ token });
  } catch (error32) {
    console.error(`[${requestId}] Error in attendee login:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
var init_login2 = __esm({
  "api/login.ts"() {
    ...
    init_validation();
    init_errors();
    __name2(onRequestOptions37, "onRequestOptions");
...
__name(onRequestOptions38, "onRequestOptions38");
async function onRequestGet24(context22) {
  ...
  try {
    ...
    if (attendeeData.group_id) {
      const { results: memberResults } = await context22.env.DB.prepare(`
    ...
    const announcements = await getAttendeeAnnouncements(context22.env.DB, ref, attendeeData.group_id, attendeeData.group_name);
    let familyRegistration = null;
    if (attendeeData.email) {
      const { results: regRows } = await context22.env.DB.prepare(`
    ...
    for (const row of teamRows) {
      const { results: teamMembers } = await context22.env.DB.prepare(`
    ...
    console.error(`[${requestId}] Error in /api/me:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function getAttendeeAnnouncements(db, _ref, groupId, groupName) {
  try {
    ...
    console.error("Error fetching attendee announcements:", error32);
    return [];
  }
}
...
__name(isNewAnnouncement2, "isNewAnnouncement2");
...
var init_me = __esm({
  "api/me.ts"() {
    ...
    init_auth();
    init_errors();
    __name2(onRequestOptions38, "onRequestOptions");
    __name2(onRequestGet24, "onRequestGet");
    __name2(getAttendeeAnnouncements, "getAttendeeAnnouncements");
    __name2(isNewAnnouncement2, "isNewAnnouncement");
    __name2(getTypeBadge, "getTypeBadge");
    __name2(getPriorityBadge, "getPriorityBadge");
  }
});
async function onRequestOptions39() {
  return handleCORS();
}
__name(onRequestOptions39, "onRequestOptions39");
async function onRequestPut8(context22) {
  const requestId = generateRequestId();
  try {
    const auth = await checkAttendeeAuth(context22.request, context22.env.JWT_SECRET || context22.env.ADMIN_JWT_SECRET);
    if (!auth) {
    ...
    console.error(`[${requestId}] Profile update error:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
var init_profile = __esm({
  "api/profile.ts"() {
    ...
    init_auth();
    init_errors();
    __name2(onRequestOptions39, "onRequestOptions");
...
__name(onRequestOptions40, "onRequestOptions40");
async function onRequestGet25(context22) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error fetching registration info:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function onRequestPost21(context22) {
  ...
  try {
    ...
    const { results: existingAttendee } = await context22.env.DB.prepare(`
    ...
    const emailPromise = sendRegistrationNotification(context22.env, data, result.meta.last_row_id).catch((err) => console.error(`[${requestId}] Failed to send notification email:`, err));
    context22.waitUntil(emailPromise);
    return createResponse({
      success: true,
    ...
    console.error(`[${requestId}] Error submitting registration:`, error32);
    return createErrorResponse(handleError(error32, requestId));
  }
}
...
async function sendRegistrationNotification(env22, data, registrationId) {
  if (!env22.RESEND_API_KEY || !env22.FROM_EMAIL) {
...
var init_register = __esm({
  "api/register.ts"() {
    ...
    __name2(onRequestOptions40, "onRequestOptions");
    __name2(onRequestGet25, "onRequestGet");
    __name2(onRequestPost21, "onRequestPost");
    __name2(validateRegistration, "validateRegistration");
    __name2(calculateTotal, "calculateTotal");
    __name2(calculateAge, "calculateAge");
    __name2(getMemberTypeLabel, "getMemberTypeLabel");
    __name2(sendRegistrationNotification, "sendRegistrationNotification");
  }
});
async function onRequest(context22) {
  const requestId = `req_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 9)}`;
  context22.data = context22.data || {};
  context22.data.requestId = requestId;
  try {
    ...
    console.error(`[${requestId}] Unhandled error in middleware:`, error32);
...
// .wrangler/tmp/bundle-3iYsHW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3iYsHW/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.4514949215307493.js.map
