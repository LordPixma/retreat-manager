...
var init_auth = __esm({
  "_shared/auth.ts"() {
    "use strict";
    init_functionsRoutes_0_5710550903109928();
...
async function sendAnnouncementEmails(env, options) {
...
async function sendNotification(env, options) {
...
async function onRequestPost4(context) {
  ...
  try {
    ...
    console.error(`[${requestId}] Error sending bulk emails:`, error);
    return createErrorResponse(handleError(error, requestId));
  }
}
...
function getStripe(env) {
  ...
  if (!stripeInstance) {
    stripeInstance = new stripe_esm_worker_default(env.STRIPE_SECRET_KEY, {
...
async function sendTeamNotificationEmails(env, teamId, teamName, description, memberIds, leaderId, requestId) {
  if (!env.RESEND_API_KEY || !env.FROM_EMAIL || memberIds.length === 0)
    return;
  ...
  try {
    ...
    if (body.member_ids !== void 0) {
      ...
      if (addedIds.length > 0) {
        const teamName = body.name?.trim() || (await context.env.DB.prepare("SELECT name FROM activity_teams WHERE id = ?").bind(id).all()).results[0];
...
async function sendConfirmation(env, attendeeId, amountPence, requestId) {
  try {
    if (!env.RESEND_API_KEY || !env.FROM_EMAIL)
      return;
...
async function sendApprovalEmail(env, email, primaryName, attendees, totalAmount, paymentOption) {
  if (!env.RESEND_API_KEY || !env.FROM_EMAIL) {
...
async function onRequestPost9(context) {
  ...
  try {
    ...
    const { name, email, ref_number, password, room_id, group_id, payment_due } = body;
    const { results: existing } = await context.env.DB.prepare(
...
async function onRequestGet18(context) {
  ...
  try {
    ...
    const countStmt = statusFilter ? context.env.DB.prepare(countQuery).bind(statusFilter) : context.env.DB.prepare(countQuery);
    const { results: countResult } = await countStmt.all();
    const total = countResult[0].total;
    const stmt = statusFilter ? context.env.DB.prepare(dataQuery).bind(statusFilter, limit, offset) : context.env.DB.prepare(dataQuery).bind(limit, offset);
  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed":
        await handleCheckoutCompleted(context, event.data.object, requestId);
        break;
      case "checkout.session.expired":
        await handleCheckoutExpired(context, event.data.object, requestId);
        break;
      default:
...
async function handleCheckoutCompleted(context, session, requestId) {
...
async function handleCheckoutExpired(context, session, requestId) {
  await context.env.DB.prepare(`
...
async function sendPaymentConfirmationEmail(env, attendeeId, amountPence, paymentType, requestId) {
  try {
    if (!env.RESEND_API_KEY || !env.FROM_EMAIL)
      return;
    const { results } = await env.DB.prepare(
...
async function onRequestPost20(context) {
  ...
  try {
    ...
    const emailPromise = sendRegistrationNotification(context.env, data, result.meta.last_row_id).catch((err) => console.error(`[${requestId}] Failed to send notification email:`, err));
    context.waitUntil(emailPromise);
    return createResponse({
      success: true,
...
async function sendRegistrationNotification(env, data, registrationId) {
  if (!env.RESEND_API_KEY || !env.FROM_EMAIL) {
...
// ../.wrangler/tmp/pages-0CiCLu/functionsRoutes-0.5710550903109928.mjs
var routes;
var init_functionsRoutes_0_5710550903109928 = __esm({
  "../.wrangler/tmp/pages-0CiCLu/functionsRoutes-0.5710550903109928.mjs"() {
    "use strict";
    init_email();
    init_email();
